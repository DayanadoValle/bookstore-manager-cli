...
export class EmprestimoController {
  constructor(
    private readonly emprestimoService: EmprestimoService = new EmprestimoService(),
  ) {}
  ...
  private async registrarEmprestimo(): Promise<void> {
    ...
    if (dataInput.trim()) {
      if (!validarData(dataInput.trim())) {
        throw new AppError(
          "Data inválida. Use o formato AAAA-MM-DD (ex.: 2026-07-20).",
        );
      }
  ...
  private async consultarPorId(): Promise<void> {
    ...
    console.log(
      `Devolução real: ${formatarData(emprestimo.dataDevolucaoReal)}`,
    );
    const statusExibicao = emprestimo.estaAtrasado()
      ? "atrasado 🔴"
      : emprestimo.status;
