...
export class LivroController {
  ...
  private async cadastrar(): Promise<void> {
    ...
    const anoPublicacao = this.lerInteiroOpcional(
      readlineSync.question("Ano de publicação (opcional): "),
      "ano de publicação",
    );
  ...
  private async listar(): Promise<void> {
    ...
    livros.forEach((l) =>
      console.log(
        `${l.id} | ${l.titulo} | ${l.autorNome ?? "-"} | ${l.quantidadeDisponivel}/${l.quantidadeTotal}`,
      ),
    );
  }
  ...
  private async consultarPorId(): Promise<void> {
    ...
    console.log(
      `Disponível/Total: ${livro.quantidadeDisponivel}/${livro.quantidadeTotal}`,
    );
  }
  ...
  private async atualizar(): Promise<void> {
    ...
    const novaQuantidadeStr = readlineSync.question(
  ...
  private lerInteiroNaoNegativo(valor: string, campo: string): number {
    ...
    if (
      ...
      throw new AppError(
        `O campo "${campo}" deve ser um número inteiro válido e não negativo.`,
      );
    }
    return numero;
  }
  ...
  private lerInteiroOpcional(valor: string, campo: string): number | undefined {
    ...
    if (isNaN(numero) || !Number.isInteger(numero)) {
      throw new AppError(
        `O campo "${campo}" deve ser um número inteiro válido.`,
      );
    }
    return numero;
  }
