...
export class AutorController {
  ...
  private async cadastrar(): Promise<void> {
    ...
    const nacionalidade =
      readlineSync.question("Nacionalidade (opcional): ") || undefined;
    const dataNascimentoInput = readlineSync.question(
  ...
  private async listar(): Promise<void> {
    ...
    autores.forEach((a) =>
      console.log(
        `${a.id} | ${a.nome} | ${a.nacionalidade ?? "-"} | ${formatarData(a.dataNascimento)}`,
      ),
    );
  }
  ...
  private async atualizar(): Promise<void> {
    ...
    const novaDataNascimentoInput = readlineSync.question(
      "Nova data de nascimento AAAA-MM-DD: ",
    );

    const nome = novoNome.trim() ? novoNome : atual.nome;
    const nacionalidade = novaNacionalidade.trim()
      ? novaNacionalidade
      : atual.nacionalidade;
  ...
  private validarDataOpcional(valor: string): string | undefined {
    ...
    if (!validarData(valor.trim())) {
      throw new AppError(
        "Data inválida. Use o formato AAAA-MM-DD (ex.: 1994-10-12).",
      );
    }
    return valor.trim();
  }
