...
export class EmprestimoService {
  ...
  async registrarEmprestimo(dados: EmprestimoDTO): Promise<Emprestimo> {
    ...
    if (!livro) {
      throw new AppError(
        `Não é possível registrar o empréstimo: livro com id ${dados.livroId} não existe.`,
      );
    }
    ...
    if (!cliente) {
      throw new AppError(
        `Não é possível registrar o empréstimo: cliente com id ${dados.clienteId} não existe.`,
      );
    }
    ...
    if (dados.dataDevolucaoPrevista) {
      ...
      if (dados.dataDevolucaoPrevista < hojeISO) {
        throw new AppError(
          "A data de devolução prevista não pode estar no passado.",
        );
      }
    }
    ...
    if (!livro.possuiDisponibilidade()) {
      throw new AppError(
        `O livro "${livro.titulo}" não possui unidades disponíveis para empréstimo.`,
      );
    }

    const possuiEmprestimoAtivo =
      await this.emprestimoRepository.clientePossuiEmprestimoAtivo(
        dados.clienteId,
      );
  ...
  async registrarDevolucao(id: number): Promise<Emprestimo> {
    ...
    if (!atualizado) {
      throw new AppError(
        `Não foi possível registrar a devolução do empréstimo com id ${id}.`,
      );
    }
