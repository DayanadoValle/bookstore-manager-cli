...
export class LivroService {
    ...
    async cadastrar(dados: LivroDTO): Promise<Livro> {
        ...
        if (dados.quantidadeTotal === undefined || dados.quantidadeTotal < 0) {
            throw new AppError('A quantidade total do livro deve ser um número válido e não negativo.');
        }

        const autor = await this.autorRepository.buscarPorId(dados.autorId);
        if (!autor) {
            throw new AppError(`Não é possível cadastrar o livro: autor com id ${dados.autorId} não existe.`);
        }
    ...
    async atualizar(id: number, dados: LivroDTO): Promise<Livro> {
        ...
        if (dados.quantidadeTotal === undefined || isNaN(dados.quantidadeTotal) || dados.quantidadeTotal < 0) {
            throw new AppError('A quantidade total do livro deve ser um número válido e não negativo.');
        }

        const autor = await this.autorRepository.buscarPorId(dados.autorId);
        if (!autor) {
            throw new AppError(`Não é possível atualizar o livro: autor com id ${dados.autorId} não existe.`);
        }
        ...
        const atualizado = await this.livroRepository.atualizar(id, dados, novaQuantidadeDisponivel);
        if (!atualizado) {
    ...
    async remover(id: number): Promise<void> {
        ...
        const possuiVinculo = await this.livroRepository.possuiEmprestimosVinculados(id);
        if (possuiVinculo) {
            throw new AppError('Não é possível remover o livro pois há empréstimos vinculados a ele.');
        }
